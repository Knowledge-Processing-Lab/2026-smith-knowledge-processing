...
clean_data <- raw_data |> 
  ...
  modify_at("E1", mutate,
    ...
    scenario = if_else(
      ...
      "TB-K-B", type_of_stimuli
    ) 
  ) |>
  
  # Specify if a trial is experimental or filler 
  modify_at(c("E1", "E2"), mutate, 
  ...
  map(mutate,
    statement_type = fct(statement_type, levels = c("Reality", "Knowledge", "Belief")),
    statement_number = as.integer(statement_number)
  ) |> 

  # Remove unused columns 
  map(select,
    p_id, statement_type, rt, accuracy, experimental, scenario, stimuli_type, stimulus, 
    truth_value, statement_number, statement, why_false, any_of("negated"), # E3 only
